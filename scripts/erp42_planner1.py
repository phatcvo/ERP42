...
class erp_planner():
    def __init__(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.is_status==True  and self.is_obj==True:
                ...
                self.print_info()
                # print("position___ :",local_path)
                
                # print('out_vel: {}'.format(self.global_path.poses))
    ...
    def print_info(self):
        ...
        os.system('clear')
        print('--------------------status-------------------------')
        print('position 1:{0}, {1}, {2}'.format(self.status_msg.position.x, self.status_msg.position.y, self.status_msg.position.z))
        ...
        # animation
            # plt.cla()
            # plt.plot(node.x, node.y, marker='.', color='r')
            ...
            # # for m in range(len(self.global_path.poses)):
            # #     draw.Arrow(states[m][0], states[m][1], np.deg2rad(states[m][2]), 2, 'darkviolet')

            # plt.axis("equal")
            # plt.title("vel =" + str(self.status_msg.velocity)[:1] + "km/h")
