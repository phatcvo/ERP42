...
# from tracking_control import purePursuit
# from vel_control import cruiseControl, velocityPlanning, velocityController
# from perception import vaildObject
import numpy as np
import tf
...
class main_planner:
    def __init__(self):
        ...
        # Publisher
        self.ctrl_cmd_pub = rospy.Publisher("/ctrl_cmd", CtrlCmd, queue_size = 1) # Vehicle Control Ctrl Cmd Publisher
        self.traffic_pub = rospy.Publisher("/SetTrafficLight", SetTrafficLight, queue_size = 1)
        self.local_path_pub = rospy.Publisher('/local_current_path',Path, queue_size=1)
        
        self.ctrl_cmd_msg = CtrlCmd() # Publisher to ROS msg by MORAI Definition
        self.ctrl_cmd_msg.longlCmdType = 1                    
        # longCmdType == 1: Throttle control. Use only accel/brake/steering
        # longCmdType == 2: Velocity control. Use only velocity/steering
        # longCmdType == 3: Acceleration control. Use only acceleration/steering
        ...
        self.tracking_control = tracking_control()
        ...
        while not rospy.is_shutdown():
            ...
            # if self.is_path == True and self.is_status == True and self.is_global_path == True:                                      
                ...
                lattice_path_num = self.lattice.select_lane(global_npc_info, global_obs_info, lattice_path)

                # Speed control using PID (target Velocity, Status Velocity)
                # if self.is_lattice_planning:
                #     output = self.pid.pid(min(target_vel/2, cc_vel), self.status_msg.velocity.x)
                ...
                if self.is_traffic == True:
                    ...
                    self.traffic_pub.publish(self.set_traffic_data)
                ...
                # Speed control using PID (target Velocity, Status Velocity)
                # output = self.pid.pid(min(target_vel, cc_vel), self.status_msg.velocity.x)
                output = self.pid.pid(target_vel, self.status_msg.velocity.x)
                ...
                print("speed control", self.ctrl_cmd_msg.accel)
                self.local_path_pub.publish(lattice_path[lattice_path_num])
...
class tracking_control:
    ...
    def getPath(self, msg):
        self.local_path = msg  #nav_msgs/Path 
    ...
    def steering_angle(self):
        ...
        t = np.array([  [cos(self.vehicle_yaw),  -sin(self.vehicle_yaw), translation[0]],
        ...
        # for _, i in enumerate(local_path_point.poses): # Path data read  
...
class latticePlanner:
    ...
    def latticePlanner(self,ref_path, vehicle_status):
        ...
        look_distance = int(vehicle_velocity*0.9*2*5)
        
        if look_distance < 5 : #min 10m   
            look_distance = 5            
        elif look_distance > 45:
            look_distance = 45
        ...
        if len(ref_path.poses) > look_distance:
            ...
            add_point_size=int(vehicle_velocity * 2 * 3.6) * 3  
