...
def Steer_Conv(steer0_b, steer1_b):
    ...
    # print(steer*np.pi/180)  # 1 is 0.014 degree, so 71 is 1 degree
    # print('steer: ', steer)
    return steer
...
def Encoder_Conv(enc0_b, enc1_b, enc2_b, enc3_b):
    ...
    if (encoder3[0] & 0x80000000) == 0:
        ...
        encoder = -2 ** 32 + 1 + (encoder0[0] + (2 ** 8) * encoder1[0] + (2 ** 16) * encoder2[0] + (2 ** 24) * (
                    encoder3[0] & 0x7fffffff))
...
def Parsing():
    ...
    if (len(Pdata) == 18):
        ...
        print("The length of data incorrect")
    print('gear:', gear, 'speed: ', speed, 'steer:', steer * np.pi/180, 'brake', brake, 'encoder: ', encoder)
